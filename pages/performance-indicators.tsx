...
import { CheckCircle, ArrowLeft, ArrowRight, RotateCcw, Brain, BookOpen, Loader2, Target } from "lucide-react";
...
export default function PerformanceIndicatorsPage() {
  ...
  // Generate practice questions for the selected category
  const generatePracticeQuestions = async () => {
    ...
    try {
      ...
      const data = await apiRequest('/api/test/personalized', 'POST', {
        cluster,
        level: 'District',
          ...
          <Card className="bg-card/80 backdrop-blur-sm">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                {Object.keys(PI_CATEGORIES).map((category: string) => (
                  <Button
                    key={category}
                    ...
                    onClick={() => {
                      ...
                      resetPracticeQuiz(); // Reset practice when switching categories
                    }}
                  >
                    {category}
                  </Button>
              ...
              <div className="mt-6">
                ...
                <p className="text-xs text-muted-foreground mt-2 text-center">
                  Test your knowledge of {selectedCategory}
                </p>
              </div>
              ...
              <div className="mt-6">
                ...
                <div className="bg-background/60 backdrop-blur-sm p-3 rounded-lg">
                  ...
                  <div className="flex justify-between text-sm mb-1 mt-3">
                    ...
                    <span className="text-foreground font-medium">
                      {(stats as any)?.completedPIs || 0}/{(stats as any)?.totalPIs || 0}
                    </span>
                  </div>
                  <div className="w-full h-2 bg-muted rounded-full">
                    <div 
                      className="bg-primary h-2 rounded-full" 
                      style={{ 
                        width: `${(stats as any)?.totalPIs ? ((stats as any).completedPIs / (stats as any).totalPIs) * 100 : 0}%` 
                      }}
                    ></div>
                  </div>
                </div>
